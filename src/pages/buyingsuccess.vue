<template>
    <div class="pt pb">
        ...
        <div class="footer fs-18">
            <router-link to="orderdetail" v-on:click.native="_orderId(info.order_no)">查看订单</router-link>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        created() {
            ...
            if(this.$route.query.orderId) {
                this._orderId(this.$route.query.orderId);
            }
            this.showInfo();
        },
        computed: {
            ...mapGetters(['paysucId', 'orderId'])
        },
        methods: {
            ...mapActions(['_orderId', '_ajax']),
            showInfo() {
                this._ajax({
                    name: 'limitedBuyShow',
                    options: {
